...
LRESULT CALLBACK WndProc(HWND hWnd, UINT iMessage, WPARAM wParam, LPARAM lParam)
{
    ...
    static TCHAR temp[10][31];
    static TCHAR result[10][100];
    static int row = 0, col = 0;

    static int xPos, yPos;

    static int isInsert = 0;
    static bool isF1 = false, isF2 = false, isF3 = false, isF4 = false, isF5 = false;
    ...
    case WM_KEYDOWN:
    {
        ...
        case VK_F5:
            break;
        case VK_F6:
            lstrcpy(temp[0], str[0]);
            for (int i = 0; i < row; i++)
            {
                lstrcpy(str[i], str[i + 1]);
            }
            lstrcpy(str[row], temp[0]);
            break;
        case VK_F7:
            for (int i = 0; i < 10; i++)
            {
